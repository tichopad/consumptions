<script lang="ts">
	...
	import {
		deleteBillingPeriodFormSchema,
		type DeleteBillingPeriodForm
	} from './delete-billing-period-form-schema';

	/** Controls whether the dialog's open */
	export let open = false;
	/** Form data from superforms */
	export let data: SuperValidated<Infer<DeleteBillingPeriodForm>>;
	/** The billingPeriod */
	export let billingPeriod: BillingPeriod | null;

	const form = superForm(data, {
		validators: zodClient(deleteBillingPeriodFormSchema),
		onUpdated({ form }) {
			if (form.valid) {
				if (form.posted) {
					open = false;
				}
				toast.success(form.message ?? 'Vyúčtování bylo smazáno.');
			} else {
				toast.error(form.message ?? 'Nepodařilo se smazat vyúčtování.');
			}
		},
		onError({ result }) {
...
{#if billingPeriod !== null}
	<Dialog.Root bind:open>
		...
		<Dialog.Content class="sm:max-w-[425px]">
			<form method="post" action="?/deleteBillingPeriod" use:enhance>
				...
				<Dialog.Header>
					<Dialog.Title class="flex flex-col items-start gap-1">
						<span>Odstranit vyúčtování</span>
						<span>{startEndDateFmt(billingPeriod)}?</span>
					</Dialog.Title>
					<Dialog.Description class="py-2">
						Tato operace je <b>nevratná</b>! Záznamy spojené s tímto vyučtováním budou zachovány.
					</Dialog.Description>
				</Dialog.Header>
