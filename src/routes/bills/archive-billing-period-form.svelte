	...
	const form = superForm(data, {
		...
		onUpdated({ form }) {
			if (form.valid) {
				...
				toast.success(form.message ?? 'Vyúčtování bylo archivováno.');
			} else {
				toast.error(form.message ?? 'Nepodařilo se archivovat vyúčtování.');
			}
		},
		onError({ result }) {
...
{#if billingPeriod !== null}
	<Dialog.Root bind:open>
		...
		<Dialog.Content class="sm:max-w-[425px]">
			<form method="post" action="?/archiveBillingPeriod" use:enhance>
				...
				<Dialog.Header>
					<Dialog.Title class="flex flex-col items-start gap-1">
						<span>Archivovat vyúčtování</span>
				...
				<Dialog.Footer class="pt-3">
					...
					<Form.Button disabled={$delayed}>{$delayed ? 'Archivuji ...' : 'Ano'}</Form.Button>
				</Dialog.Footer>
			</form>
		</Dialog.Content>
	</Dialog.Root>
